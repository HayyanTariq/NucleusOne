...
export const AdminDashboard = () => {
  const { user } = useAuth();
  const { trainings, addTraining, deleteTraining, canViewAllTrainings } = useTraining();
  ...
  const isAdmin = canViewAllTrainings();
  const trainingDescription = isAdmin ? "All company training records" : "Your training records";
  const employeeDescription = isAdmin ? "Active employees" : "Your profile";
  ...
  return (
    <div className="flex-1 space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Welcome back, {user?.firstName}! {isAdmin ? "Here's your company's training overview." : "Here's your training overview."}
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <StatsCard
          title={isAdmin ? "Total Employees" : "Your Profile"}
          value={isAdmin ? stats.totalEmployees : 1}
